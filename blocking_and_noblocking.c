...
struct newchar_dev{
	...
	wait_queue_head_t wait_head;	/* wait list head */
};
...
static void timer_function(unsigned long arg)
{
	printk("irq happend !\n");
	read_val = !read_val;
	wake_up_interruptible(&char_dev.wait_head);	/* wake up wait list */
}

static irqreturn_t irq_handler(int irq, void *dev)
{
	struct newchar_dev *handler_dev = (struct newchar_dev *)dev;
	mod_timer(&handler_dev->timer, jiffies + msecs_to_jiffies(30));	/* 10ms shake */
...
static ssize_t char_read(struct file *filep, char __user *buf, size_t size, loff_t *lofft)
{
	...
	ret = wait_event_interruptible(char_dev.wait_head,read_val);		/* waiting for val */
	data_buf[0] = read_val;
...
static int __init chardev_init(void)
{
	...
	init_waitqueue_head(&char_dev.wait_head);
